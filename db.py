"""
db.py — Shared database connection for DBS tool
...
def get_dbs_tool_id() -> Optional[int]:
    global _TOOL_ID
    ...
    if not session:
        return None
    try:
        result = session.execute(text("SELECT id FROM tools WHERE slug = 'dbs' LIMIT 1")).fetchone()
        if result:
            _TOOL_ID = result[0]
        return _TOOL_ID
    except Exception as e:
        log.error("[DB] get_dbs_tool_id error: %s", e)
        return None
    finally:
        session.close()
...
def create_job_record(*, tenant_id: int, user_id: int, total_items: int) -> Optional[int]:
    ...
    try:
        ...
        result = session.execute(text("""
            ...
            RETURNING id
        """), {"tenant_id": tenant_id, "user_id": user_id, "tool_id": tool_id, "total_items": total_items, "now": datetime.utcnow()})
        row = result.fetchone()
        session.commit()
...
def update_job_status(*, db_job_id: int, status: str, successful_items: int, failed_items: int) -> None:
    ...
    try:
        session.execute(text("""
            ...
            WHERE id = :job_id
        """), {"status": status, "successful": successful_items, "failed": failed_items, "now": datetime.utcnow(), "job_id": db_job_id})
        session.commit()
    except Exception as e:
...
def record_usage(*, tenant_id: int, user_id: int, db_job_id: Optional[int], successful_outputs: int) -> None:
    ...
    try:
        ...
        """), {"tenant_id": tenant_id, "user_id": user_id, "tool_id": tool_id, "job_id": db_job_id, "count": successful_outputs, "now": datetime.utcnow()})
        session.execute(text("UPDATE tenants SET tokens_used = tokens_used + :count WHERE id = :tenant_id"), {"count": successful_outputs, "tenant_id": tenant_id})
        session.commit()
    except Exception as e:
